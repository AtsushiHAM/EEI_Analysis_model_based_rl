...
class Policy_Random(object):
    ...
    def __init__(self, env):
        ...
        job = re.findall('I.?P', str(type(self.env.env)))
        if not job :
        ...
        # just uniformly sample random actions
            ...
            if (self.counter%hold_action)==0:
                ...
                job = re.findall('I.?P', str(type(
                    self.env.env.env)))
                if job[0] == 'IP':
                    #self.rand_ac = np.random.uniform(self.env.env.env.action_space.low, self.env.env.env.action_space.high, self.shape)
                    self.rand_ac = np.random.uniform(self.low_val, self.high_val, self.shape)
                else:
                    self.rand_ac = np.random.uniform(self.low_val, self.high_val, self.shape)"""
            action = self.rand_ac
