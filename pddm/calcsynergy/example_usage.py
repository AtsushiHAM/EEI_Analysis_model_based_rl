...
parser.add_argument('--folder_path',
                    type=str,
                    default="example_timeseries/HC_r4_actions",
...
for count,npy_file in enumerate(sorted_files):
    ...
    print("torque shape {}".format(torque_npy.shape))
    ####call the funciton from core.py#############
    R2_list_single_line, surface_area_single_line=calculate_synergy(torque_npy,synergy_type, start_index,windows)
    ###############################################

    all_label.append('Checkpoints ' + str((count+1)))
