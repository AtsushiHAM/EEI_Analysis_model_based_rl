...
def vis_iter_graph(args, load_dir0):
    ...
    for vis_index in range(len(rollouts_info0)):
        ...
        for ind in range(actions0.shape[0]):
            new_energy = np.abs(states0[ind, 2] * actions0[ind, 0])
            new_error = states0[ind, 1] ** 2
            new_energy2= states0[ind, 2] * actions0[ind, 0]
            new_error2 = np.abs(states0[ind, 1])
            energy= energy + new_energy
            error = error + new_error
            enrrgy2 = energy2 + new_energy2
        ...
        if args.eval:
            np.save(save_dir + "/eval_eeis_{}.npy".format(args.iter_num), np.array(EEI3))
            np.save(save_dir + "/eval_ers_{}.npy".format(args.iter_num), np.array(error2))
            ...
            np.save(save_dir + "/eeis_{}.npy".format(args.iter_num), np.array(EEI3))
            np.save(save_dir + "/ers_{}.npy".format(args.iter_num), np.array(error2))
