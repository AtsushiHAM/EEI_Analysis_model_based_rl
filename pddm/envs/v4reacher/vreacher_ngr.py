...
class VReacherEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self, file_path=os.path.join(GYM_ASSET_PATH, "vertical_arm4dof_ngr.xml"), max_step=1000):
        self.time = 0
        self.num_step = 0
...
class VReacherEnv4(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self, file_path=os.path.join(GYM_ASSET_PATH, "vertical_arm4dof_ngr.xml"), max_step=1000):
        self.time = 0
        self.num_step = 0
        self.max_step = max_step  # maximum number of time steps for one episode
    ...
    def _get_obs(self):
        ...
        self.obs_dict['joints_force'] = np.asarray([self.data.sensordata[1],self.data.sensordata[4],self.data.sensordata[7],
                                                    self.data.sensordata[10]]).flat.copy()
        # self.sim.data.qpos.flat[2:].copy()#self.get_body_com("fingertip") - self.get_body_com("target").copy()
        #self.obs_dict['goal_vel'] = self.sim.data.qvel.flat[2:].copy()
        #theta = self.sim.data.qpos.flat[:2]
        return np.concatenate([
