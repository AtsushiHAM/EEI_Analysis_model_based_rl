...
class InvertedPendulumEnv1(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self,file_path=os.path.join(GYM_ASSET_PATH,"default_ip.xml"),max_step=1000):
        self.time = 0
        self.num_step =0
...
class InvertedPendulumEnv1_1(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self,file_path=os.path.join(GYM_ASSET_PATH,"default_ip.xml"),max_step=1000):
        self.time = 0
        self.num_step =0
        self.max_step=max_step #maximum number of time steps for one episode
    ...
    def _get_obs(self):
        ...
        self.obs_dict['joints_vel'] = self.sim.data.qvel.flat.copy()
        self.obs_dict['joints_force'] = np.asarray(
            [self.data.sensordata[1]]).flat.copy()
        return np.concatenate([
            self.obs_dict['joints_pos'], #2
            self.obs_dict['joints_vel'], #2
            self.obs_dict['joints_force']
...
class InvertedPendulumEnv1_2(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self,file_path=os.path.join(GYM_ASSET_PATH,"default_ip.xml"),max_step=1000):
        self.time = 0
        self.num_step =0
        self.max_step=max_step #maximum number of time steps for one episode
    ...
    def _get_obs(self):
        ...
        self.obs_dict['joints_vel'] = self.sim.data.qvel.flat.copy()
        self.obs_dict['joints_force'] = np.asarray(
            [self.data.sensordata[2]]).flat.copy()
        return np.concatenate([
            self.obs_dict['joints_pos'], #2
            self.obs_dict['joints_vel'], #2
            self.obs_dict['joints_force']
...
class InvertedPendulumEnv2(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self,file_path=os.path.join(GYM_ASSET_PATH,"default_ip.xml"),max_step=1000):
        self.time = 0
        self.num_step =0
        self.max_step=max_step #maximum number of time steps for one episode
...
class InvertedPendulumEnv5(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def _get_obs(self):
        ...
        self.obs_dict['joints_pos'] = self.sim.data.qpos.flat.copy()
        self.obs_dict['joints_vel'] = self.sim.data.qvel.flat.copy()
        self.obs_dict['joints_force'] = np.asarray([self.data.sensordata[0],self.data.sensordata[1],self.data.sensordata[2],self.data.sensordata[3],self.data.sensordata[4]]).flat.copy()
        return np.concatenate([
            self.obs_dict['joints_pos'], #2
            self.obs_dict['joints_vel'], #2
            self.obs_dict['joints_force']
...
class InvertedPendulumEnv3_124(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self,file_path=os.path.join(GYM_ASSET_PATH,"default_ip.xml"),max_step=1000):
        self.time = 0
        self.num_step =0
        self.max_step=max_step #maximum number of time steps for one episode
    ...
    def _get_obs(self):
        ...
        self.obs_dict['joints_pos'] = self.sim.data.qpos.flat.copy()
        self.obs_dict['joints_vel'] = self.sim.data.qvel.flat.copy()
        self.obs_dict['joints_force'] = np.asarray([self.data.sensordata[1],self.data.sensordata[2],self.data.sensordata[4]]).flat.copy()
        return np.concatenate([
            self.obs_dict['joints_pos'], #2
            self.obs_dict['joints_vel'], #2
            self.obs_dict['joints_force']
